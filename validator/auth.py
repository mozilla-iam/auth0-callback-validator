...
class Auth():
    ...
    def get_auth0_vars(self):
        ...
        self.mgmt_domain = config.get_auth0_domain()
        self.mgmt_client_id = config.get_auth0_client_id()
        self.mgmt_client_secret = config.get_auth0_client_secret()
    ...
    def auth0_connect(self):
        try:
            if self.mgmt_api_token is None:
                get_token = auth0.authentication.GetToken(self.mgmt_domain, self.mgmt_client_id, self.mgmt_client_secret)
            ...
            if self.auth_client == None:
                self.auth_client = auth0.management.Auth0(self.mgmt_domain, self.mgmt_api_token)
